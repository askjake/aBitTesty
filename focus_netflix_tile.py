#!/usr/bin/env python3
"""
focus_netflix_tile.py

Navigate around the Home screen until the red-outlined tile that contains
the Netflix logo is in focus, then stop (and optionally SELECT).

Invocation & command-line handling is identical to the original
Focus_Active_Recording.py:

  python focus_netflix_tile.py <PortMask> [-f TEXT] \
         <JobRunHistoryID> <UIJobHistoryID> <UITestCaseHistoryID> <UIStepHistoryID>

The optional -f switch lets you *also* require some text in the right-hand
info pane (handy if multiple profiles / rows are visible).
"""
...
_LOGO_TMPL = str(pathlib.Path(__file__).with_name("netflix_logo_166x100.png"))
# ^ a crisp crop of the Netflix word-mark on transparent/black background.
#   (≈166×100 px in 1920×1080 capture – adjust if you capture at a
...
def _ensure_template() -> np.ndarray:
    """Load the reference logo once (cached)."""
    tmpl = cv2.imread(_LOGO_TMPL, cv2.IMREAD_UNCHANGED)
    if tmpl is None:
        raise FileNotFoundError(f"Logo template {_LOGO_TMPL!s} not found")
    # get 3-channel BGR (drop alpha if present)
    if tmpl.shape[2] == 4:
        tmpl = cv2.cvtColor(tmpl, cv2.COLOR_BGRA2BGR)
    return tmpl

_TMPL = _ensure_template()

def _label_says_netflix(frame: np.ndarray,
                        rect: tuple[int,int,int,int],
                        dbg_prefix: str | None = None) -> bool:
    """
    OCR the little label just below the highlighted tile.
    Returns True if we see 'NETFLIX' in that label.
    """
    x, y, w, h = rect
    # take a slice immediately under the tile:
    y1 = y + h + int(0.05 * h)
    y2 = y + h + int(0.60 * h)
    x1, x2 = x, x + w
    label_roi = frame[y1:y2, x1:x2]
    if dbg_prefix:
        _save_dbg(f"{dbg_prefix}_label_roi.png", label_roi)

    # gray + Otsu threshold to isolate white/red on dark
    gray = cv2.cvtColor(label_roi, cv2.COLOR_BGR2GRAY)
    _, bw = cv2.threshold(gray, 0, 255,
                          cv2.THRESH_BINARY + cv2.THRESH_OTSU)

    # OCR letters only
    cfg = "--oem 3 --psm 7 -c tessedit_char_whitelist=ABCDEFGHIJKLMNOPQRSTUVWXYZ"
    txt = pytesseract.image_to_string(bw, config=cfg).strip().upper()
    if dbg_prefix:
        print(f"[OCR {dbg_prefix}] label -> {txt!r}")
    return "NETFLIX" in txt.split()

def _tile_has_netflix_logo(tile: np.ndarray,
                           dbg_prefix: str | None = None) -> bool:
    """
    Template‐match the Netflix logo inside the tile. Returns True if
    normalized match ≥ 0.40.
    """
    ...
    # invert so bright red dark shapes on white
    _, b1 = cv2.threshold(t1, 180, 255, cv2.THRESH_BINARY_INV)
    _, b2 = cv2.threshold(t2, 180, 255, cv2.THRESH_BINARY_INV)
    ...
    if dbg_prefix:
        h2, w2 = b2.shape
        tl = (int(loc[0]), int(loc[1]))
        br = (tl[0] + w2, tl[1] + h2)
        dbg = tile.copy()
        cv2.rectangle(dbg, tl, br, (0,255,0), 2)
        _save_dbg(f"{dbg_prefix}_tmpl_{mx:.2f}.png", dbg)
        print(f"[TEMPLATE {dbg_prefix}] score={mx:.2f}")

    return mx >= 0.40


def _highlight_is_netflix(frame: np.ndarray,
                          hi_rect: tuple[int,int,int,int],
                          dbg_prefix: str | None = None) -> bool:
    """
    Convenience wrapper: crop the highlighted tile out of the frame and call
    the logo matcher above.
    """
    x,y,w,h = hi_rect
    # inset a hair so you don't include the red border itself
    tile = frame[y+4:y+h-4, x+4:x+w-4]
    return _tile_has_netflix_logo(tile, dbg_prefix)

# ─────────────────────────────────────────────────────────────────────────────
#  MAIN NAVIGATION LOOP
# ─────────────────────────────────────────────────────────────────────────────
def navigate_to_netflix(
    mask: int,
    filter_text: str | None = None,
    max_passes: int = 3
) -> None:
    """
    Navigate the DVR grid until the Netflix tile is highlighted, then press SELECT.
    Recognition succeeds if either:
      • the Netflix logo is template-matched inside the red highlight, or
      • the “Netflix” text label just below the tile OCRs correctly.
    Raises AssertionError if not found after max_passes.
    """
    # 1) grab first frame to detect the red highlight rectangle
    ...
    for nav_pass in range(max_passes):
        ...
        for row in range(grid_rows):
            for col in range(grid_cols):
                debug_prefix = f"net_{nav_pass}_{row}_{col}"
                frame = grab_current_frame(mask)

                # re‐find the red highlight each time
                ...
                # 1) logo match?
                logo_ok = _tile_has_netflix_logo(tile, debug_prefix)
                # 2) label OCR?
                text_ok = _label_says_netflix(frame, rect, debug_prefix)

                if logo_ok or text_ok:
                    print(f"[OK] Netflix found at pass={nav_pass}, row={row}, col={col} "
                          f"(logo={logo_ok}, text={text_ok})")
                    #send_command_ex(mask, "SELECT")
                    return
    ...
    raise AssertionError(f"Could not locate Netflix tile after {max_passes} passes")


# ───────────────────────────────────────────────────────────────────────────
#  Main entry – same CLI contract as before
# ───────────────────────────────────────────────────────────────────────────
def main() -> None:
    # pull off optional -f TEXT
    args = sys.argv[1:]
    pane_filter = None
    if "-f" in args:
        idx = args.index("-f")
        pane_filter = args[idx+1]
        del args[idx:idx+2]

    if not args:
        print("Usage: focus_netflix_tile.py <PortMask> [-f TEXT] [...]")
        sys.exit(1)

    try:
        port_mask = int(args[0])
    except ValueError:
        print("PortMask must be an int (1,2,4,8,...)")
        sys.exit(1)

    UIStepHistoryID = args[4] if len(args) >= 5 else None

    try:
        navigate_to_netflix(port_mask, pane_filter)
    ...
    print("[OK] Netflix tile FOUND.")
    # send_command_ex(port_mask, "SELECT")   # optional
    sys.exit(0)
...
if __name__ == "__main__":
    try:
        ...
        traceback.print_exc()      # print full stacktrace to stdout/stderr
        sys.exit(1)
